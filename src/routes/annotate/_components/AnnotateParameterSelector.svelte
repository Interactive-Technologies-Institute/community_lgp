...
<Popover.Root bind:open>
	<Popover.Trigger
		class={cn(
			'flex h-8 w-full min-w-0 flex-row items-center justify-center gap-2 rounded-lg border border-brand-border bg-brand-white px-2 text-foreground dark:bg-brand-surface',
			open && 'border-brand-blue text-brand-blue'
		)}
	>
		Selecionar
		{#if open}
					...
					{#if parent.is_parent && (parent.image || ['mão', 'braço', 'frente', 'tronco'].includes(parent.name?.toLowerCase() ?? '') || parent.code?.toLowerCase() === '2ebj')}
						...
						>
							<Card.Content class="flex min-w-0 flex-col items-center gap-2 p-2">
								...
								{#if parent.code}
									<div
										class="grid w-full gap-1 {parent.tipo === 'localizacao'
											? 'grid-cols-2'
											: 'grid-cols-3'}"
									>
